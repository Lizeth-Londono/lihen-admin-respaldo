-- LIHEN ADMIN - MIGRACIÓN 010
-- Corrección definitiva: la tabla audit_logs de este proyecto no tiene columna old_data.
-- Reemplaza update_order_atomic_v2 conservando la edición transaccional.
-- Ejecutar UNA SOLA VEZ después de la migración 009.
...
begin
  ...
  -- La tabla audit_logs real solo dispone de new_data y details.
  -- Guardamos el estado anterior dentro de details para conservar trazabilidad
  -- sin provocar un rollback por la columna inexistente old_data.
  insert into public.audit_logs(
    ...
    jsonb_build_object(
      ...
      'source', 'update_order_atomic_v2_migration_010'
    )
  );
