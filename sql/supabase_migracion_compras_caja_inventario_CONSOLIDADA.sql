...
-- ============================================================
-- INICIO BLOQUE: 032_compatibilidad_plantilla_inventario_2026_08_07.sql
-- ============================================================
-- LIHEN.CO - Compatibilidad segura con LIHEN-INVENTARIO-V1
-- Ejecutar después de 031. No carga datos del Excel ni altera inventario por sí sola.
begin;

create table if not exists public.product_cost_import_history (
  id uuid primary key default gen_random_uuid(),
  product_id uuid not null references public.products(id) on delete restrict,
...
create index if not exists product_cost_import_history_product_date_idx
  on public.product_cost_import_history(product_id, created_at desc);
create index if not exists product_cost_import_history_batch_idx
  on public.product_cost_import_history(import_batch_id)
  where import_batch_id is not null;

alter table public.product_cost_import_history enable row level security;
alter table public.product_cost_import_history force row level security;
drop policy if exists cofundadoras_consultan_product_cost_import_history on public.product_cost_import_history;
create policy cofundadoras_consultan_product_cost_import_history
  on public.product_cost_import_history for select to authenticated
  using (public.is_active_cofounder());
revoke all on public.product_cost_import_history from anon;
revoke insert, update, delete, truncate, references, trigger on public.product_cost_import_history from authenticated;
grant select on public.product_cost_import_history to authenticated;
...
-- FIN BLOQUE: 032_compatibilidad_plantilla_inventario_2026_08_07.sql
