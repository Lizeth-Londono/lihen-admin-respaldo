-- ============================================================
-- LIHEN ADMIN — MIGRACIÓN 048
-- Hotfix definitivo: elimina falso conflicto de concurrencia en supplier_name
-- y mantiene control optimista para campos de products y stock.
-- Ejecutar DESPUÉS de la migración 043.
-- ============================================================
...
begin
  ...
  loop
    ...
    if v_action = 'update' then
      ...
      -- HOTFIX 048: supplier_name no participa en el snapshot optimista.
      -- La relación de proveedor vive en supplier_products (no en products) y la RPC
      -- no reemplaza de forma destructiva al proveedor preferido. Comparar aquí el
      -- nombre calculado por la vista previa podía producir falsos conflictos por
      -- orden/relaciones múltiples aunque el producto no hubiera cambiado.
