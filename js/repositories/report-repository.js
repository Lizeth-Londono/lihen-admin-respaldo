...
export async function fetchReportSources() {
  const results = await Promise.all([
    ...
    supabase.from('products').select('id,name,current_cost,sale_price,visible_on_website,business_line,category,subcategory'),
    ...
    supabase.from('supplier_request_items').select('supplier_request_id,product_id,quantity_requested,quantity_received,quoted_unit_cost'),
    supabase.from('supplier_payments').select('id,supplier_request_id,supplier_id,financial_account_id,amount,payment_date,status,payment_source,account:financial_accounts(id,name,code)'),
